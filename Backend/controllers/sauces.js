...
exports.createSauce = (req, res, next) => {
    ...
    const sauce = new Sauce({// Then, we call the Sauces constructor which is in the models to build the object by retrieving all the parameters that are present in the sauce constant
        ...sauceObject,
...
exports.getOneSauce = (req, res, next) => {
    Sauce.findOne({// We retrieve via findOne the sauce corresponding to the linked ID in the database.
    ...
    const sauceObject = req.file ?
        {
            ...
            imageUrl: `${req.protocol}://${req.get('host')}/images/${req.file.filename}` // We retrieve the image file (if an image is sent) so that it contains the path with the typed file name (Ex: img.jpg)

        } : { ...req.body };// We update the data received, with an image or without.

    Sauce.updateOne({ _id: req.params.id }, { ...sauceObject, _id: req.params.id })// Finally we update the database object via the save property of mongoose.
...
exports.deleteSauce = (req, res, next) => {
    Sauce.findOne({ _id: req.params.id }) // We call the findOne function to retrieve the unique ID of the sauce created by the user,
        .then(sauce => { 
            ...
            fs.unlink(`images/${filename}`, () => { // Then with the fs package, we will look for the corresponding immage file in the server tree and we delete it
...
exports.getAllSauces = (req, res, next) => {
    Sauce.find().then(// We retrieve all the elements of the "Table / Collection" Sauce from the database,
        (sauces) => {
